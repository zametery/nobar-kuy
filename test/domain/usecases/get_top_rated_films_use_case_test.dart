...
void main() => testGetTopRatedFilmsUseCase();

void testGetTopRatedFilmsUseCase() {
  late FilmRepository repository;
  late GetTopRatedFilmUseCase useCase;

  setUp(() {
    repository = MockFilmRepository();
    useCase = GetTopRatedFilmUseCase(repository);
  });

  group('Get Popular Use Case -', () {
    group('Given film type.', () {
      group('When call getTopRatedFilmsUseCase, ', () {
        test('Then it return success', () async {
          when(repository.getTopRatedFilms(FilmType.movie)).thenAnswer(
